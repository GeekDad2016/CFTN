...
def train_rl(args):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch % 5 == 0:
            with torch.no_grad():
                # We save a grid: Top 4 = Greedy, Bottom 4 = Sampled
                # We must limit to batch size if batch size < 4
                num_vis = min(4, len(captions))
                ...
                # --- NEW: Format Captions for WandB ---
                # We take the prompts corresponding to the visualized images
                vis_caps = captions[:num_vis]
                # Join them with newlines for better readability in WandB
                caption_str = f"Epoch {epoch} Prompts:\n" + "\n".join([f"{i+1}. {c}" for i, c in enumerate(vis_caps)])
